...
ORYX runs as a lightweight daemon (~9 MB RAM) that checks your server every minute, detects anomalies (disk spikes, auth bruteforces, service failures), attempts automatic fixes, and only alerts you via Telegram when it's truly stuck. It also has an interactive chat TUI for on-demand diagnostics.
...
```bash
# Download (choose one)
curl -fsSL https://raw.githubusercontent.com/syawalqi/ORYX/master/install.sh | bash

# Or manually:
curl -fsSL https://github.com/syawalqi/oryx/releases/download/v1.1.0/ORYX-linux-amd64 -o ORYX
...
```bash
git clone https://github.com/syawalqi/oryx.git
cd ORYX
