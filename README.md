...
- **Command palette** — `/help`, `/scan`, `/config`, `/memory`, `/save`, `/model`, `/update`
...
- **Inline reflection** — LLM reasons and self-corrects within the same response
...
# Or manual download (check releases for latest version)
curl -fsSL https://github.com/syawalqi/ORYX-AI/releases/latest/download/oryx-linux-amd64 -o oryx
...
| `oryx --update` | Self-update (stable or dev track) |
...
| `/update` | Check and apply updates (--dev, --force) |
...
```yaml
provider: openrouter              # LLM provider (openrouter, opencode-go, anthropic, ollama)
api_key: sk-...                   # Your API key
model: google/gemini-2.0-flash    # Model for chat and fix agent
daemon_model: google/gemini-2.0-flash  # Model for daemon auto-fix
...
- **OpenRouter** — `openrouter.ai/api/v1` (200+ models, default)
